<script lang="ts">
  ...
  const log = (label: string, payload?: unknown, intent: ActivityIntent = 'info') => {
    ...
    logEntries = [
      { id: ++logCounter, ts: new Date().toLocaleTimeString(), label, intent, payload: formatted },
      ...logEntries,
    ].slice(0, 200)
  }
  ...
  const pingHost = async () => {
    const response = await ping('Demo ping from example app')
    log('ping', response ?? 'null')
  }
  ...
  const requestNewDevice = async () => {
    ...
    const hasFilterDetails = servicesFilter.length > 0 || filterName.trim().length > 0 || filterNamePrefix.trim().length > 0
  ...
  const fetchCharacteristics = async () => {
    ...
    characteristics = await getCharacteristics(deviceId, workingServiceUuid, workingCharacteristicUuid || undefined)
  ...
  const readCharacteristic = async () => {
    ...
    const result = await readCharacteristicValue(deviceId, workingServiceUuid, workingCharacteristicUuid)
  ...
  const writeCharacteristic = async () => {
    ...
    await writeCharacteristicValue(deviceId, workingServiceUuid, workingCharacteristicUuid, payload, withResponse)
  ...
  const startNotify = async () => {
    ...
    log('start_notifications', { deviceId, serviceUuid: workingServiceUuid, characteristicUuid: workingCharacteristicUuid })
  }
  ...
  const stopNotify = async () => {
    ...
    log('stop_notifications', { deviceId, serviceUuid: workingServiceUuid, characteristicUuid: workingCharacteristicUuid })
  }
  ...
  const isSubscribed = () => {
    ...
    return subscribedKeys.includes(subKey(activeDeviceId, workingServiceUuid, workingCharacteristicUuid))
  }
...
<main class="app-shell">
  <header class="hero">
    <div>
      ...
      <p>Trigger every plugin command, inspect payloads, and watch events without leaving this starter app.</p>
    </div>
  ...
  <section class="grid">
    <section class="panel span-2">
      <div class="panel-heading">
        ...
        <span class={`chip ${availability === null ? 'chip-muted' : availability ? 'chip-ok' : 'chip-warn'}`}>{availabilityLabel}</span>
      </div>
      ...
      <div class="session-grid">
        <article>
          ...
          {#if currentDevice}
            ...
            <p class="pill-list">Services: {currentDevice.uuids?.length ?? 0} · Connected: {currentDevice.connected ? 'yes' : 'no'}</p>
          {:else}
        ...
        <article>
          <h3>Notifications</h3>
          <p class="muted">Listeners for characteristic changes and GATT disconnects stay attached.</p>
          <p class={`pill ${isSubscribed() ? 'pill-live' : ''}`}>
            {isSubscribed() ? 'Subscribed to current characteristic' : 'No active subscription for current selection'}
          </p>
        </article>
      </div>
    </section>
                ...
                <ul>
                  {#each service.characteristics as characteristic}
                    <li>
                      ...
                      <span class="muted">
                        {#if characteristic.properties.notify}notify {/if}
                        {#if characteristic.properties.read}read {/if}
                        {#if characteristic.properties.write}write {/if}
                      </span>
                    </li>
                  {/each}
                </ul>
              {:else}
                <p class="muted">No characteristics returned (use get_characteristics for more data).</p>
              {/if}
            </article>
          {/each}
        </div>
    ...
    <section class="panel span-2">
      ...
      <div class="field">
        ...
        <textarea rows="3" placeholder="Type raw text, base64, or hex" bind:value={writeValue}></textarea>
      </div>
  ...
  import { ping } from 'tauri-plugin-web-bluetooth-api'
